...
advanced_more() {
    while true; do
        ...
        echo ""
        echo "2) Webcam Auto-Config (camera-streamer) - FPS fix & Device Number"
        echo ""
...
extract_model_and_motor() {
 ...
 install_printer_cfg() {
    ...
    if [[ "$DIFF_CHOICE" =~ ^[Yy]$ ]]; then
        ...
        DIFF_OUTPUT=$(diff -y --suppress-common-lines --width=58 "${HOME}/OpenOrangeStorm/printer-confs/output.cfg" "${HOME}/printer_data/config/printer.cfg")
            ...
            case $opt in
                1)
                    ...
                    for file in "${config_files[@]}"; do
                        if [[ -n $file ]]; then
                            cp "${HOME}/OpenOrangeStorm/img-config/printer-data/$file" "${HOME}/printer_data/config/"
                ...
                2|3|4|5|6|7|8)
                    ...
                    if [[ "$DIFF_CHOICE" =~ ^[Yy]$ ]]; then
                        ...
                        if [[ $file == "data.mdb" ]]; then
                            DIFF_OUTPUT=$(diff -y --suppress-common-lines --width=58 "${HOME}/OpenOrangeStorm/img-config/printer-data/$file" "${HOME}/printer_data/database/$file")
                        else
                            DIFF_OUTPUT=$(diff -y --suppress-common-lines --width=58 "${HOME}/OpenOrangeStorm/img-config/printer-data/$file" "${HOME}/printer_data/config/$file")
                        fi
...
print_menu() {
    ...
    echo "=========================================================="
    echo ""
    echo "1) Install/Update OpenOrangeStorm printer.cfg"
    echo ""
