...
SCRIPT="${HOME}/OpenOrangeStorm/OpenOrangeStorm.sh"
DISPLAY_SERVICE_INSTALLER="${HOME}/display_connector/display-service-installer.sh"
MCU_RPI_INSTALLER="${HOME}/OpenOrangeStorm/img-config/rpi-mcu-install.sh"
USB_STORAGE_AUTOMOUNT="${HOME}/OpenOrangeStorm/img-config/usb-storage-automount.sh"
ANDROID_RULE_INSTALLER="${HOME}/OpenOrangeStorm/img-config/adb-automount.sh"
...
DE_ELEGOO_IMAGE_CLEANSER="${HOME}/OpenOrangeStorm/img-config/de_elegoo_cleanser.sh"
...
OpenOrangeStorm_DIR="${HOME}/OpenOrangeStorm"
...
# ASCII art for OpenOrangeStorm
OpenOrangeStorm_ART=$(cat <<'EOF'
  ____                ____                         ______               
...
run_fixes() {
    ...
    # Create a symbolic link for OpenOrangeStorm Logo in fluidd
    ln -s ${HOME}/OpenOrangeStorm/pictures/logo_OpenOrangeStorm.svg ${HOME}/fluidd/logo_OpenOrangeStorm.svg > /dev/null 2>&1
    # Create a symbolic link to the main script if it doesn't exist
    SYMLINK_PATH="/usr/local/bin/openorangestorm"
...
[Service]
Type=simple
ExecStart=/home/mks/OpenOrangeStorm/img-config/power_monitor.sh
User=root
Restart=on-failure
RestartSec=5s
...
set_current_branch() {
    current_branch=$(git -C "$OpenOrangeStorm_DIR" symbolic-ref --short HEAD 2>/dev/null)
}
...
update_repo() {
    ...
    echo "=========================================================="
    echo ""
    process_repo_update "$OpenOrangeStorm_DIR" "OpenOrangeStorm"
    moonraker_update_manager "OpenOrangeStorm"
    if [ -d "${HOME}/OpenOrangeStorm/display/venv" ]; then
        ...
        if [[ $user_input =~ ^[Yy]$ ]]; then
            ...
            rm -r "${HOME}/OpenOrangeStorm/display"
        else
...
moonraker_update_manager() {
    ...
    if [ "$update_selection" = "OpenOrangeStorm" ]; then
        new_lines="[update_manager $update_selection]\n\
type: git_repo\n\
primary_branch: $current_branch\n\
path: $OpenOrangeStorm_DIR\n\
is_system_service: False\n\
origin: $OpenOrangeStorm_REPO"
...
origin: $DISPLAY_CONNECTOR_REPO"
        ...
        echo -e "${R}Invalid argument. Please specify either 'OpenOrangeStorm' or 'display_connector'.${NC}"
        return 1
    fi
...
advanced_more() {
    while true; do
        ...
        echo -e "${C}$OpenOrangeStorm_ART${NC}"
        echo "=========================================================="
        echo -e "              OpenOrangeStorm - ${M}Advanced Options${NC} "
        echo "=========================================================="
        echo ""
        ...
        echo ""
        echo "4) Update OpenOrangeStorm Repository"
        echo ""
        ...
        echo ""
        echo -e "7) Elegoo Image Cleanser Script (NOT for OpenOrangeStorm)"
        echo -e "----------------------------------------------------------${NC}"
        echo ""
...
de_elegoo_image_cleanser() {
    install_feature "Elegoo Image/eMMC Cleanser" "$DE_ELEGOO_IMAGE_CLEANSER" "DO NOT run this on an OpenOrangeStorm GitHub Image, for Elegoo images only! Do you want to proceed?"
}
    ...
    if [ -d "$OpenOrangeStorm_DIR" ]; then
        if [ -n "$current_branch" ]; then
            ...
            if [[ $user_response =~ ^[Yy]$ ]]; then
                switch_branch "$target_branch" "$OpenOrangeStorm_DIR"
            ...
            echo -e "${R}Could not determine the current branch for $OpenOrangeStorm_DIR.${NC}"
        fi
    else
        echo -e "${R}$OpenOrangeStorm_DIR does not exist or is not accessible.${NC}"
    fi
}
...
install_printer_cfg() {
    ...
    if [[ $model_key == "n4" || $model_key == "n4pro" ]]; then
        ...
        DTB_SOURCE="${HOME}/OpenOrangeStorm/dtb/n4-n4pro-v${pcb_version}/rk3328-roc-cc.dtb"
        ...
        DTB_SOURCE="${HOME}/OpenOrangeStorm/dtb/n4plus-n4max-v1.1-2.0/rk3328-roc-cc.dtb"
...
apply_configuration() {
    ...
    if [ "$install_configs" = "true" ]; then
        ...
        sleep 1
        if cp -r ~/OpenOrangeStorm/img-config/printer-data/* ~/printer_data/config/ && \
    ...
    if [[ -n "$DTB_SOURCE" && -f "$DTB_SOURCE" ]]; then
        ...
        if $update_dtb && ! grep -q "mks" /boot/.OpenOrangeStorm.txt; then
            ...
            sleep 1
        elif grep -q "mks" /boot/.OpenOrangeStorm.txt; then
...
OpenOrangeStorm configuration script.
...
Commands:
  install_printer_cfg        Install or update the OpenOrangeStorm Printer.cfg and other configurations.
  ...
  update_repo                Update the OpenOrangeStorm repository to the latest version.
...
print_menu() {
    ...
    echo "=========================================================="
    echo ""
    echo "1) Install/Update OpenOrangeStorm printer configurations"
    echo ""
...
TEMP=$(getopt -o yh --long yes,help,printer_model:,motor_current:,pcb_version: -n 'OpenOrangeStorm.sh' -- "$@")
